...
fn match_rust() {
    ...
    match number {
        1 | 3 => println!("number is 1"),
        10 | 20 => println!("number is 10"),
...
fn shadowing_rust() {
    ...
    let x = 5;
    println!("x={}", x);
    let x = "tanmay kumar charuasia";
    println!("x={}", x);
    let x = x.len();
    println!("x={}", x);
}

fn dynamic_array_vector() {
    let mut v: Vec<i8> = Vec::new(); // vector declaration
    let mut nums = Vec::<String>::new(); // another way of declaring vector

    nums.push(String::from("tanmay"));
    nums.push(String::from("kumar"));
    nums.push(String::from("charuasia"));

    v.push(1);
    v.push(2);
    v.push(3);

    let mut nums1 = vec![1, 2, 3, 4, 5, 6]; // initializing the vector with the values
    nums1.push(10);

    println!("value of vector v is: {:?}", v);
    println!("value of vector nums is: {:?}", nums); // value of vector nums is: ["tanmay", "kumar", "charuasia"]
    println!("value of vector nums1 is: {:?}", nums1); //value of vector nums1 is: [1, 2, 3, 4, 5, 6, 10]
}
